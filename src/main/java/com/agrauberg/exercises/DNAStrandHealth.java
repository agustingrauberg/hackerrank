...
public class DNAStrandHealth {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.printf("%d %d", dnaHealth.minHealth(), dnaHealth.maxHealth());
        } catch (IOException e) {
...
class Dna {
    ...
    int parseGene(int id, String str, int start, long health) {
        ...
        Node node = root;
        letter = str.charAt(i);
        for (; i < str.length() && Character.isAlphabetic(letter); ++i) {
            Node next = node.findChild(letter, null);
            if (next == null) {
                next = new Node(node, letter);
                node.addChild(next);
            }
            node = next;
        }
...
class GenesHealth {
    private final int id; // single value optimization
    int idMin;
    int idMax;
    ...
    long health(int first, int last) {
        ...
        long r = 0;
        for (long thisHealth : values.subMap(first, true, last, true).values()) {
            r += thisHealth;
        }
        return r;
    }
